...
def add_exercises_to_plan():
    ...
    try:
        ...
        if not plan_id:
            ...
            insert_plan_query = "INSERT INTO plan (user_user_id1, level, plan_name, status, AI) VALUES (%s, %s, %s, %s, %s)"
            my_cursor.execute(insert_plan_query, (user_id, level, plan_name, "Active", False))
            mydb.commit()
...
def get_user_allworkouts():
    ...
    sql_plans = """
        SELECT p.plan_id, p.level, p.plan_name, p.AI
        FROM plan p
...
def fakeAi():
    try:
        ...
        query = "INSERT INTO plan (user_user_id1, goal, level, plan_name, AI) VALUES (%s, %s, %s, %s, %s)"
        values = (user_id, goal, level, plan_name, True)  # Set the AI column to True
